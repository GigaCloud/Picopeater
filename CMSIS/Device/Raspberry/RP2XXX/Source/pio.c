...
static float pio_get_div(uint32_t div)
{
    uint16_t integer = div >> PIO0_SM0_CLKDIV_INT_Pos;
    float    frac    = (float)((div & 0xFFFF) >> PIO0_SM0_CLKDIV_FRAC_Pos) / 256.0f;
    return (integer + frac);
}
...
{
    float divider = (float) peri_clk / (float) requested_clk;
    if (divider < 1.0f) {
        divider = 1.0f;
    } else if (divider > UINT16_MAX) {
        divider = UINT16_MAX;
    }
    ...
    float f_div = pio_get_div(clkdiv_value);
    return (peri_clk / f_div);
}
