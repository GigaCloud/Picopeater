...
#ifndef PIO_INSTRUCTIONS
    #define PIO_INSTRUCTIONS 0
#endif
...
extern "C"
{
    ...
    inline static uint16_t _pio_encode_instr_and_args(enum pio_instr_bits instr_bits, uint16_t arg1, uint16_t arg2)
    {
    ...
    inline static uint16_t _pio_encode_instr_and_src_dest(enum pio_instr_bits instr_bits, enum pio_src_dest dest, uint16_t value)
    {
        return _pio_encode_instr_and_args(instr_bits, dest & 7u, value);
    }

    inline static uint16_t pio_encode_delay(uint16_t cycles)
    {
        valid_params_if(PIO_INSTRUCTIONS, cycles <= 0x1f);
        return cycles << 8u;
    }

    inline static uint16_t pio_encode_sideset(uint8_t sideset_bit_count, uint16_t value)
    {
    ...
    inline static uint16_t pio_encode_sideset_opt(uint8_t sideset_bit_count, uint16_t value)
    {
    ...
    inline static uint16_t pio_encode_jmp(uint16_t addr)
    {
        return _pio_encode_instr_and_args(pio_instr_bits_jmp, 0, addr);
    }

    inline static uint16_t _pio_encode_irq(bool relative, uint16_t irq)
    {
        valid_params_if(PIO_INSTRUCTIONS, irq <= 7);
        return (relative ? 0x10u : 0x0u) | irq;
    }

    inline static uint16_t pio_encode_wait_gpio(bool polarity, uint16_t pin)
    {
        return _pio_encode_instr_and_args(pio_instr_bits_wait, 0u | (polarity ? 4u : 0u), pin);
    }

    inline static uint16_t pio_encode_wait_pin(bool polarity, uint16_t pin)
    {
        return _pio_encode_instr_and_args(pio_instr_bits_wait, 1u | (polarity ? 4u : 0u), pin);
    }

    inline static uint16_t pio_encode_wait_irq(bool polarity, bool relative, uint16_t irq)
    {
        valid_params_if(PIO_INSTRUCTIONS, irq <= 7);
        return _pio_encode_instr_and_args(pio_instr_bits_wait, 2u | (polarity ? 4u : 0u), _pio_encode_irq(relative, irq));
    }

    inline static uint16_t pio_encode_in(enum pio_src_dest src, uint16_t value)
    {
    ...
    inline static uint16_t pio_encode_out(enum pio_src_dest dest, uint16_t value)
    {
    ...
    inline static uint16_t pio_encode_push(bool if_full, bool block)
    {
        return _pio_encode_instr_and_args(pio_instr_bits_push, (if_full ? 2u : 0u) | (block ? 1u : 0u), 0);
    }

    inline static uint16_t pio_encode_pull(bool if_empty, bool block)
    {
        return _pio_encode_instr_and_args(pio_instr_bits_pull, (if_empty ? 2u : 0u) | (block ? 1u : 0u), 0);
    }

    inline static uint16_t pio_encode_mov(enum pio_src_dest dest, enum pio_src_dest src)
    {
    ...
    inline static uint16_t pio_encode_mov_not(enum pio_src_dest dest, enum pio_src_dest src)
    {
        valid_params_if(PIO_INSTRUCTIONS, !(dest & _PIO_INVALID_MOV_DEST));
        valid_params_if(PIO_INSTRUCTIONS, !(src & _PIO_INVALID_MOV_SRC));
    ...
    inline static uint16_t pio_encode_mov_reverse(enum pio_src_dest dest, enum pio_src_dest src)
    {
        valid_params_if(PIO_INSTRUCTIONS, !(dest & _PIO_INVALID_MOV_DEST));
        valid_params_if(PIO_INSTRUCTIONS, !(src & _PIO_INVALID_MOV_SRC));
    ...
    inline static uint16_t pio_encode_irq_set(bool relative, uint16_t irq)
    {
        return _pio_encode_instr_and_args(pio_instr_bits_irq, 0, _pio_encode_irq(relative, irq));
    }

    inline static uint16_t pio_encode_irq_clear(bool relative, uint16_t irq)
    {
        return _pio_encode_instr_and_args(pio_instr_bits_irq, 2, _pio_encode_irq(relative, irq));
    }

    inline static uint16_t pio_encode_set(enum pio_src_dest dest, uint16_t value)
    {
    ...
    inline static uint16_t pio_encode_nop()
    {
